...
func _process(_delta: float) -> void:
	...
	if not Global.player: if not end_step and Global.table.get_child_count() > 0:
		for i in Global.table.get_children(): if not i.security_card and i.attack:
		...
		if Global.table.get_child_count() == 0: _shot()
			...
			var end: bool = true
			for i in Global.table.get_children(): if i.attack and not i.security_card:
				end = false
				break
			if end:
...
# Ход компьютера
func _shot() -> void:
	if $"../Trash".get_child_count() > 0 and Global.table.get_child_count() > 0 : return
